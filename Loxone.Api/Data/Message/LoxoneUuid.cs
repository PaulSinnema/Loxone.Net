...
namespace Loxone.Api.Data.Message
{
    public class LoxoneUuid
    {
        ...
        public LoxoneUuid(Span<byte> rawData)
        {
            ...
            data2.Reverse();
            data3.Reverse();
            var data1Str = Utils.ByteArrayToString(data1).Replace(" ", "");
            var data2Str = Utils.ByteArrayToString(data2).Replace(" ", "");
            var data3Str = Utils.ByteArrayToString(data3).Replace(" ", "");
            var data4Str = Utils.ByteArrayToString(data4).Replace(" ", "");
